                    ...
                    <tbody>
                        @foreach ($cart_products as $cart)
                            <tr>
                                ...
                                @endif
                                <td data-cart_id="{{ $cart->id }}"
                                    data-price="{{$cart->product->discount_price ?? $cart->product->price}}">
                                    <div class="input-group quantity mt-4" style="width: 100px;">
                                        <div class="input-group-btn">
                                            <button class="btn btn-sm btn-minus rounded-circle bg-light border cart-pm">
                                        ...
                                        <div class="input-group-btn">
                                            <button class="btn btn-sm btn-plus rounded-circle bg-light border cart-pm">
    ...
    @push('scripts')
        ...
        <script>
            $(document).on('click', '.cart-pm', function () {
                ...
                var price = parseFloat($(this).parents('td').data('price'));
                var product_price = parseFloat(price * quantity);
            ...
            function totalPayout() {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            response.total_payout = parseFloat(response.total_payout).toFixed(2);
